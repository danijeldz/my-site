...
const renderImage = ({ src = "", fit = "", w = 100, h = 100 } = {}) => `${src.replace(/downloads./g, "images.")}?fit=${fit}&w=${w}&h=${h}`

const getCoverImage = (node) => {
	return node.heroImage ? node.heroImage.file : node.media.length > 0 ? node.media[0].file : null
}
